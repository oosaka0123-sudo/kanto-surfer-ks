...
function renderChart(slots){
  ...
  const col=getColWidth(),slotCount=Math.max(slots.length,1),width=Math.max(slotCount*col,(chartScroll?.clientWidth||scroll?.clientWidth||320)),height=240,top=22,bottom=32,usable=height-top-bottom;
  ...
  const xAt=i=>i*col+col/2,maxWave=Math.max(1,...waves.filter(Number.isFinite)),maxWind=Math.max(6,...winds.filter(Number.isFinite));
